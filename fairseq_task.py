...
class FairseqTask(object):
    ...
    def train_step(
        ...
        # Added to print out parameter sensitivity
        ...
        for name, params in model.named_parameters():
            ...
            if params.requires_grad:
                ...
                importances[num-1].extend(score.tolist()) 
                #mu = torch.mean(scores)
                #sigma = torch.std(scores)
                #param_importance_dict[name] = (mu.item(), sigma.item())

        #sorted_dict = sorted(param_importance_dict.items(), key=lambda x: x[1][0])
        #for item in sorted_dict:
        #    print(item[0])
        #    print(f"mean = {param_importance_dict[item[0]][0]}")
        #    print(f"std = {param_importance_dict[item[0]][1]}")
        normalized_importances = []
        for i in range(24):
            layer_importance = np.mean(importances[i-1])
            print(f" layer {i}, score {layer_importance}")
            normalied_importances.append(layer_importance)

        normalized_importances = torch.tensor(normalized_importances, devide='cuda')
        ...
        for name, params in model.named_parameters():
            ...
            if len(layer_num) <= 3 or layer_num[3].isnumeric() == False:
                ...
                if layer_num[1] == 'decoder':
                    num = num * 2
            if params.requires_grad:
                params = F.dropout(params, p=normalized_importances[num-1].item())
        """
